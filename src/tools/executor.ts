  ...
  return new Promise((resolve, reject) => {
    ...
    proc.on("error", (err) => {
      ...
      logger.error(
        ...
        err,
        { command: actualCommand, args, executable: AGENT_BROWSER_PATH },
        sessionId
      );
...
export async function execBrowser(
  ...
  if (command === "waitfordownload") {
    const args: string[] = ["wait", "--download"];
  ...
  const args: string[] = actualCommand.split(" ");

  // Define which parameters should be positional arguments for each command
  const positionalParams: Record<string, string[]> = {
    open: ["url"],
    click: ["selector"],
    fill: ["selector", "value"],
    type: ["selector", "text"],
  ...
  const addedPositional = new Set<string>();
  ...
  for (const [key, value] of Object.entries(options)) {
    if (value === undefined || value === null || addedPositional.has(key)) continue;
