...
export function registerBrowserTools(server) {
    ...
    server.tool("browser_download", `Click to trigger download. ${USE_SNAPSHOT}`, {
        ...
        return executeTool("download", { selector, path, sessionId }, () => execBrowser("download", { selector, path }, sessionId), sessionId);
    });
    server.tool("browser_wait_for_download", "Wait for auto-triggered download (use after click if download starts automatically)", {
        ...
        return executeTool("wait_for_download", { path, timeout, sessionId }, () => execBrowser("waitfordownload", { path, timeout }, sessionId), sessionId);
    });
}
